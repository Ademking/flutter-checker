...
if (package_name && !filename) {
  ...
  const buffer = await downloadFile(package_name) as unknown as Buffer
...
async function isFileExists(filename: string, zip: any) {
  const entries = zip.getEntries() as AdmZip.IZipEntry[]
